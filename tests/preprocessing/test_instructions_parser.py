...
def test_call_frame_parsing():
    ...
    def dummy_event_at_depth(depth: int):
        return TraceEvent(1234, op_from_class(POP), ["0x1122"], depth)
    ...
    test_events = [
        ...
        TraceEvent(1234, op_from_class(CALL), ["0x0", "0x0", "0x0", "0x0", "0x0", addr["call"], "0x0"], 1),
        dummy_event_at_depth(2),
        TraceEvent(1234, op_from_class(DELEGATECALL), ["0x0", "0x0", "0x0", "0x0", addr["delegatecall"], "0x0"], 2),
        dummy_event_at_depth(3),
        TraceEvent(1234, op_from_class(STATICCALL), ["0x0", "0x0", "0x0", "0x0", addr["staticcall"], "0x0"], 3),
...
def test_call_frame_updates_on_depth_change():
    ...
    test_events = [
        ...
        TraceEvent(1235, op_from_class(POP), [], depth=2),
    ]
...
def test_call_frame_makes_exceptional_halt():
    ...
    test_events = [
        TraceEvent(1234, op_from_class(CALL), ["0x0", "0x0", "0x0", "0x0", "0x0", call_target, "0x0"], depth=1),
        TraceEvent(1234, op_from_class(POP), [], depth=2),
        TraceEvent(1235, op_from_class(POP), [], depth=1),
    ]
...
def test_call_frame_parsing_throws_on_unexpected_depth_change():
    test_events = [
        TraceEvent(1234, op_from_class(POP), [], depth=1),
        TraceEvent(1235, op_from_class(POP), [], depth=2),
    ]
